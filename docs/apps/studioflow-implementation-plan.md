...
| **SF-F5** Brands global | Brand discovery melalui Master Data public port: hashtag, brand, category/brand-category; this is StudioFlow's only Master Data read | Pencarian hashtag, brand, dan kategori menghasilkan brand yang benar tanpa direct cross-schema read |
...
| **SF-F7** Product Catalogue/Schedule + MOM | StudioFlow-owned Product Catalogue reused across projects, project Schedule snapshots, and project-owned MOM using their approved contracts | Project detail exposes FFNI/Schedule and MOM without forcing either into the phase task model; Product Catalogue remains reusable across StudioFlow projects |
