...
| Upload endpoints write to runtime disk. The general media endpoint accepts a caller-selected `folder` and only checks the subsequent path. | `src/app/api/upload/media/route.ts`, `upload/library/route.ts`, `upload/temp/route.ts`, `deliverable-storage.ts`. | The endpoint has an unsafe storage-boundary design and error responses expose raw error text. | **PURGE** all caller-selected upload routes. **REUSE** the shared `ObjectStorage` capability with the approved self-hosted local provider, server-owned keys, configured root, and authenticated/private reads. |
...
- Never restore unsafe local upload endpoints, caller-selected paths, or public
  temporary URLs. Approved self-hosted storage must go through the shared
  `ObjectStorage` port with server-owned keys and private application reads.
