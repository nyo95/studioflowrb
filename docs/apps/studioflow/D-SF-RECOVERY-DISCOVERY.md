...
Status: **DISCOVERY COMPLETE — requires Planner ratification before any SF-A
through SF-F implementation**
...
| `src/app/(dashboard)/projects/[id]/activity/page.tsx`; `src/app/(dashboard)/activity/page.tsx`; `src/app/(dashboard)/activity-center/page.tsx`; `src/lib/services/task-feed.ts` `fromActivity`, `fromChecklistTask`, `groupTasksByProject`, `bucketTasksByDate` | Legacy has project activity plus global activity and date-bucketed feed views over activities and checklist tasks. | KEEP for project work; DECISION_REQUIRED for global surfaces | Project contract §10.2 supplies one rebuild work list. Planner must decide whether legacy “Activity”, “Activity Center”, and “Upcoming/Today” need separate routes or are merged into it. |
...
| `src/app/(dashboard)/projects/[id]/extensions/product-catalog/page.tsx`; `src/extensions/sketchup/components/CatalogBoard.tsx`, `ProductScheduleTabs.tsx`; `prisma/schema.prisma` `ProjectProductRequest`, `ProjectScheduleEntry`, `ProjectScheduleOption` | Product/schedule UI is scoped to a project and persists project-linked request, schedule-entry, and option records. | ALREADY_REPLACED for project scope; DECISION_REQUIRED for existing global data | `studioflow-schedule-contract.md` must be corrected/ratified around project scope before SF-C. Existing rebuild global Product Catalogue data has no legacy-derived migration policy; Planner must decide retain, re-scope, archive, or discard. |
| `src/app/(dashboard)/projects/[id]/mom/page.tsx`; `src/extensions/mom/actions/mom-actions.ts`; `src/extensions/mom/services/mom-service.ts` | Project MOM documents, ordered items/points/images, project-scope assertions, membership guard, invalidation, and audit. | ALREADY_REPLACED for ownership/lifecycle; REDESIGN for access implementation | `studioflow-mom-contract.md` owns draft/issue/supersede. Preserve project ownership and ordered content; use registered MOM grants rather than legacy membership. Planner must ratify any parity detail absent from the current lifecycle. |
| `src/app/(dashboard)/projects/[id]/sketchup/page.tsx`; `src/extensions/sketchup/actions/sketchup-actions.ts` `generateApiKeyAction`, `queueSketchupMaterialMergeAction`, `normalizeSketchupMaterialCodesAction`; `src/extensions/sketchup/lib/catalog-ownership.ts` | Project integration creates/revokes a plugin key and manages material-code reconciliation/merge queue. | REDESIGN | Deferred SketchUp exchange in `studioflow.md` §6. Do not expose raw API keys or revive direct plugin/database coupling. A future work order needs authenticated integration, idempotency, retries, reconciliation, secret lifecycle, and audit. |
| `src/app/(dashboard)/settings/profile/page.tsx`; `settings/clients/page.tsx`; `settings/studio/page.tsx`; `settings/database/page.tsx`; `src/actions/settings-actions.ts`, `database-actions.ts` | Settings has profile, client/studio configuration, and an admin database snapshot/restore screen. | ALREADY_REPLACED for platform profile; DECISION_REQUIRED for StudioFlow settings and database screen | Profile belongs to platform. Planner must explicitly classify each studio/client setting ownership. Database Settings is not a recoverable StudioFlow feature; it requires a platform operations decision and must stay absent from app navigation. |
| `src/app/(dashboard)/projects/[id]/layout.tsx`; `src/components/project-layout-shell.tsx`; `src/components/project-live-provider.tsx`, `phase-live-provider.tsx` | Project navigation shell and live providers supply workspace navigation and realtime presence/update plumbing. | KEEP for shell; DECISION_REQUIRED for live/chat | Reuse the app shell/navigation pattern through UI Engine. No project realtime/chat is activated until Planner defines user outcome, privacy, retention, delivery guarantee, and shared-vs-app ownership. |
...
| ID | Decision required | Why discovery cannot decide it | Blocking scope |
|---|---|---|---|
| D-SF-01 | Merge or retire legacy global Activity, Activity Center, and Upcoming/Today surfaces. | The pin proves overlapping feeds, not which user workflow is still required. | SF-A navigation/work list. |
| D-SF-02 | Set StudioFlow settings ownership and whether any app settings remain after profile is platform-owned. | The pin has profile, client, studio, and DB pages but does not establish current product ownership. | F-C/SF-A settings navigation. |
| D-SF-03 | Confirm MOM parity beyond the implemented draft/issue/supersede contract. | The pin proves ordered content/images and membership gating; current contract deliberately changed lifecycle and authorization. | SF-C only if a gap is identified. |
| D-SF-04 | Choose disposition for existing rebuild global Product Catalogue data. | Pin proves legacy project-local schedule data; it cannot authorize migration, deletion, or reassignment of rebuild data. | SF-C; no schema/data action before decision. |
| D-SF-05 | Decide whether project live/realtime and chat have a first-release outcome. | Providers prove legacy plumbing, not necessary user value, retention, or privacy policy. | Any live/chat work; not SF-A by default. |
| D-SF-06 | Decide whether a SketchUp exchange is activated and its operational/security contract. | Pin exposes long-lived plugin key and queue mechanics, which are not an approved secure integration design. | Dedicated post-foundation work order only. |
| D-SF-07 | Define legacy route compatibility window and redirect set. | Pin supplies old paths but cannot determine supported bookmarks, users, or retirement date. | SF-A route rollout. |
...
One factual status correction is now durable: the prior global Product
Catalogue reuse-pool premise is not recovery authority. The pin’s project
route and `project_id` records establish project-scoped product/schedule
behavior. This does **not** authorize a change to existing rebuild data or
schema; D-SF-04 remains required.

Discovery is complete as a read-only gate. Planner/Reviewer must ratify this
report and either lock the seven decisions above or issue one narrow correction
plan. Only then may SF-A through SF-F receive implementation work orders.
