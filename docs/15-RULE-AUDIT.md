...
Scope: documentation, schema, implementation claims, and reference behavior. No product code was changed by this audit.
...
|---|---|---|---|---|---|
| RA-01 | P0 | CONFLICTED | Rebuild enforces one global SkuPrice per SKU, while final product law is one current price per SKU × supplier. | Rebuild `prisma/schema.prisma` has `Sku.price SkuPrice?` and unique `SkuPrice.sku_id`; public tests assert singular `price`. Final reference contract and current owner direction require multi-supplier reuse. | Before BQ work: migrate schema constraint/cardinality, application repository/service, workbook uniqueness, UI, DTO to `prices[]`, and regression tests. Preserve in-place current state per pair and audit history. |
...
| RA-07 | P1 | CONFLICTED | `MASTER_DATA.md` presents singular pricing as a locked product contract even though it is now a known incorrect implementation baseline. | Numerous singular-cardinality statements in `MASTER_DATA.md`; RA-01 evidence. | Retain for implementation provenance, but label superseded sections. `03-MASTERDATA-PRD.md` and `apps/masterdata.md` govern new work. |
