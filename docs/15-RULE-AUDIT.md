...
| RA-13 | P1 | RESOLVED 2026-08-28 | The UI Engine styling deviated from the locked `UI_ENGINE.md` §5 baseline: Tailwind CSS 4 was installed but never wired (no `postcss.config.mjs`), the engine shipped a bespoke global `ui-*` semantic-class system in `styles/engine.css`, and several Master Data pages bypassed engine components with a second `ui-*` class vocabulary (partially undefined: `ui-button--*`, `ui-toolbar`, `ui-table-container`, `--space-*`). | Pre-conversion audit of `src/platform/ui_engine` vs `UI_ENGINE.md`/`DESIGN.md` and the legacy reference `main@5fc605e` (read-only). | Done 2026-08-28: engine components rebuilt on Tailwind 4 utilities (`@theme inline` bridge in `globals.css` binds `tokens/tokens.css` vars — still the single value source; `engine.css` deleted; `base.css` keeps only global ground/focus/reduced-motion; `print.css` keeps document/print rules). `postcss.config.mjs` added per Next 16 docs. All Master Data pages migrated onto engine components; zero raw `ui-*` classes in app code. |
