# Product Roadmap by Application (ARCHIVED 2026-09-22)

> **Superseded.** Merged into [`docs/BACKLOG.md`](../BACKLOG.md) on 2026-09-22
> at owner request, consolidating this file with `review.md` and
> `knownbug.md` into one worklist. Kept here as historical evidence of what
> was open through R8.105; do not add new items to this file.
...
### Owner review 2026-09-22 — Product Schedule vs legacy audit

Recorded from the owner's live review of the rebuilt Schedule board against
legacy `CatalogBoard.tsx`. Card-field customization (the priority item) was
implemented the same session; the rest is deferred by owner decision, not an
oversight — listed here so it isn't re-discovered from scratch later.

- [ ] **Revision history for Product Schedule** (draft → issue → supersede,
  capped v1–v5, same policy MOM already has). `domain/revisions.ts` already
  documents this as staged: "shared by every document-like record (MOM now,
  Product Schedule next)." Genuinely new feature (lifecycle + revision-history
  UI), not a UX-parity fix — scope it as its own item.
- [ ] **Unit Cost field per option.** Legacy `CatalogItem.unit_cost` — no
  equivalent column on `SfScheduleOption` at all. Needs a schema
  decision (currency handling, who can see cost) before it's just "add a
  column."
- [ ] **Reference URL field per option.** Legacy `CatalogItem.url` — same gap,
  no rebuild column.
- [ ] **Export to Sheets (.xlsx).** Legacy toolbar action on the schedule
  page; rebuild has Import CSV but no export.
- [ ] **Print Document.** Legacy has a print-optimized schedule view/toolbar
  button; rebuild's `PrintButton` UI Engine pattern exists but isn't wired
  into the schedule board.
- [ ] **Bulk code-swap ("Manage codes") tool.** Legacy lets a user pick two
  items and swap their codes as a guarded bulk operation
  (`ArrowLeftRight` tool in `CatalogBoard.tsx`); rebuild only has
  single-entry move-up/down/recategorize.
