...
Required starting ref: **`masterdata-full-build-start-ui-approved-v3`**
...
This package is executable only from `masterdata-full-build-start-ui-approved-v3`. Earlier `masterdata-full-build-start*` tags, including both prior UI-approved refs, are superseded and must not be used.

1. Checkout `main` at the new PM/TL-issued Master Data start tag.
2. Resolve and record the tag hash:

```powershell
git rev-parse masterdata-full-build-start-ui-approved-v3
git status --short
```
...
```powershell
git status --short
git log --oneline masterdata-full-build-start-ui-approved-v3..HEAD
git diff --check masterdata-full-build-start-ui-approved-v3..HEAD
npm test
npm run check
npx prisma validate
npx prisma generate
