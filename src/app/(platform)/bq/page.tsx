...
export default async function BqProjectsPage() {
  ...
  if (!canRead) return <div className="grid gap-4"><PageHeader eyebrow="Bill of Quantity" title="Projects" /><SectionCard><ErrorState title="Access denied" description="You do not have permission to view BQ projects." /></SectionCard></div>;
  ...
  const [projects, deletionRequests] = await Promise.all([bqPublicRead.listProjectSummaries(), canApproveDeletion ? bqService.listProjectDeletionRequests({ grants }) : []]);
  return <div className="flex min-h-0 flex-1 flex-col gap-6 p-(--ui-page-padding)">
    ...
    {projects.length === 0 ? <SectionCard><EmptyState icon={FileText} title="No projects yet" description="Create your first BQ project." /></SectionCard> : <DirectoryShell surface fill><DataTable framed={false} density="compact" stickyHeader fill minWidth={640}>
