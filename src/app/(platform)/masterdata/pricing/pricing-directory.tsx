...
import { Button, ButtonMenu, Combobox, ConfirmDialog, CreatableSearch, DataTable, Dialog, EmptyState, Field, FormActions, IconButton, InlineError, Input, Pagination, SearchField, SectionCard, Select, SimpleTextEditor, Spinner, StatusBadge, TableBody, Text, Tooltip, type SortDirection, TableCell, TableCellContent, TableHead, TableHeader, TableRow, TableToolbar, Tabs, useFormDraftGuard, useOptionOverlay } from "@/platform/ui_engine";
...
function UpdatedMeta({ by, at }: { by: string; at: Date }) {
  ...
  return <span className="text-xs text-ink-tertiary">Updated by {by} · {new Intl.DateTimeFormat("id-ID", { dateStyle: "medium", timeStyle: "short" }).format(updatedAt)}</span>;
}
...
export function PricingDirectory(props: { materialPrices: MaterialRow[]; materialLaborPrices: WorkRow[]; laborPrices: WorkRow[]; canManageMaterial: boolean; canManageWork: boolean; canReadMaterial: boolean; canReadWork: boolean; canManageVendors: boolean; canManageCategories: boolean; canManageSkus: boolean; canManageBrands: boolean; skus: SkuRef[]; brands: Ref[]; productCategories: Ref[]; vendors: Ref[]; units: Array<Ref & { code: string }>; workCategories: Ref[]; vendorTypes: Array<Ref & { canSupplyMaterial: boolean; canSupplyLabor: boolean }> }) {
  ...
  return <SectionCard>
    ...
    <Tabs
      ...
      items={[
      { value: "material", label: `Material Prices (${props.materialPrices.length})`, disabled: !props.canReadMaterial, content: <div className="grid gap-4">{toolbar}{material.length === 0 ? <EmptyState title="No material prices" description={query ? "No prices match your search." : "Create a material price for an active SKU and eligible vendor."} /> : <><DataTable minWidth={720}><TableHeader><TableRow>{sortableHead("name", "SKU")}{sortableHead("vendor", "Supplier")}{sortableHead("amount", "Price", "end")}<TableHead>Unit</TableHead><TableHead>Status</TableHead>{props.canManageMaterial && <TableHead align="end">Actions</TableHead>}</TableRow></TableHeader><TableBody>{materialPaged.rows.map((row) => <TableRow key={row.id}><TableCell><TableCellContent primary={<span className="font-semibold">{row.sku.name ?? row.sku.code ?? "Unnamed SKU"}</span>} secondary={<div className="grid gap-0.5">{row.sku.code ? <span className="font-mono text-xs">{row.sku.code}</span> : null}<UpdatedMeta by={row.updated_by_label} at={row.updated_at} /></div>} /></TableCell><TableCell>{row.supplier_vendor.name}</TableCell><TableCell align="end">{displayPrice(row.amount, row.currency)}</TableCell><TableCell><span className="font-mono text-xs">{row.unit.code}</span></TableCell><TableCell><StatusBadge tone={row.deleted_at ? "neutral" : "success"}>{row.deleted_at ? "Archived" : "Active"}</StatusBadge></TableCell>{actions(row, { kind: "material", id: row.id, name: `${row.sku.name ?? row.sku.code ?? "Unnamed SKU"} / ${row.supplier_vendor.name}` }, props.canManageMaterial, () => setEditor({ kind: "material", row }))}</TableRow>)}</TableBody></DataTable>{pagination(materialPaged.currentPage, materialPaged.pageCount)}</>}</div> },
