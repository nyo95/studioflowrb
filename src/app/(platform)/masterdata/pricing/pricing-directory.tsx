...
export function PricingDirectory(props: { materialPrices: MaterialRow[]; materialLaborPrices: WorkRow[]; laborPrices: WorkRow[]; canManageMaterial: boolean; canManageWork: boolean; canReadMaterial: boolean; canReadWork: boolean; canManageVendors: boolean; canManageCategories: boolean; canManageSkus: boolean; canManageBrands: boolean; skus: SkuRef[]; brands: Ref[]; productCategories: Ref[]; vendors: Ref[]; units: Array<Ref & { code: string }>; workCategories: Ref[]; vendorTypes: Array<Ref & { canSupplyMaterial: boolean; canSupplyLabor: boolean }> }) {
  ...
  const workTable = (rows: WorkRow[], kind: Kind, canManage: boolean, title: string) => { const filtered = rows.filter((row) => matches(`${row.name} ${row.category.name} ${row.vendor.name}`, Boolean(row.deleted_at))); const paged = paginate(sortRows(filtered, (row) => row.name, (row) => row.vendor.name)); return <DirectoryShell fill surface toolbar={toolbar} pagination={pagination(paged.currentPage, paged.pageCount)}>{filtered.length === 0 ? <EmptyState title={title} description={query ? "No prices match this search." : "No pricing records yet."} /> : <><DataTable density="compact" stickyHeader fill framed={false} minWidth={650}><TableHeader><TableRow>{sortableHead("name", "Name")}<TableHead>Category</TableHead>{sortableHead("vendor", "Supplier")}{sortableHead("amount", "Price", "end")}<TableHead>Unit</TableHead>{canManage && <TableHead stickyEnd align="end">Actions</TableHead>}</TableRow></TableHeader><TableBody>{paged.rows.map((row) => <TableRow key={row.id}><TableCell><EntityPrimaryCell tone={row.deleted_at ? "danger" : "success"} statusLabel={row.deleted_at ? "Archived" : "Active"} name={row.name} secondary={<UpdatedMeta by={row.updated_by_label} at={row.updated_at} />} /></TableCell><TableCell>{row.category.name}</TableCell><TableCell>{row.vendor.name}</TableCell><TableCell align="end">{displayPrice(row.amount, row.currency)}</TableCell><TableCell><span className="font-ui-mono text-xs">{row.unit.code}</span></TableCell>{actions(row, { kind, id: row.id, name: row.name }, canManage, () => setEditor({ kind, row }))}</TableRow>)}</TableBody></DataTable></>}</DirectoryShell>; };
  return <div className="flex min-h-0 flex-1 flex-col gap-4">
    ...
    <Tabs
      ...
      items={[
      { value: "material", label: `Material Prices (${props.materialPrices.length})`, disabled: !props.canReadMaterial, content: <DirectoryShell fill surface toolbar={toolbar} pagination={pagination(materialPaged.currentPage, materialPaged.pageCount)}>{material.length === 0 ? <EmptyState title="No material prices" description={query ? "No prices match this search." : "Create a material price for an active SKU and eligible supplier."} /> : <><DataTable density="compact" stickyHeader fill framed={false} minWidth={610}><TableHeader><TableRow>{sortableHead("name", "SKU")}{sortableHead("vendor", "Supplier")}{sortableHead("amount", "Price", "end")}<TableHead>Unit</TableHead>{props.canManageMaterial && <TableHead stickyEnd align="end">Actions</TableHead>}</TableRow></TableHeader><TableBody>{materialPaged.rows.map((row) => <TableRow key={row.id}><TableCell><EntityPrimaryCell tone={row.deleted_at ? "danger" : "success"} statusLabel={row.deleted_at ? "Archived" : "Active"} name={row.sku.name ?? row.sku.code ?? "Unnamed SKU"} secondary={<div className="grid gap-0.5">{row.sku.code ? <span className="font-ui-mono text-xs">{row.sku.code}</span> : null}<UpdatedMeta by={row.updated_by_label} at={row.updated_at} /></div>} /></TableCell><TableCell>{row.supplier_vendor.name}</TableCell><TableCell align="end">{displayPrice(row.amount, row.currency)}</TableCell><TableCell><span className="font-ui-mono text-xs">{row.unit.code}</span></TableCell>{actions(row, { kind: "material", id: row.id, name: `${row.sku.name ?? row.sku.code ?? "Unnamed SKU"} / ${row.supplier_vendor.name}` }, props.canManageMaterial, () => setEditor({ kind: "material", row }))}</TableRow>)}</TableBody></DataTable></>}</DirectoryShell> },
