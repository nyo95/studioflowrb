...
export default async function MomPage({ params }: { params: Promise<{ id: string; momId: string }> }) {
  ...
  return <>
    <Breadcrumb entries={[{ label: "Projects", href: "/studioflow/projects" }, { label: "Project", href: `/studioflow/projects/${id}` }, { label: mom.topic }]} />
    <PageHeader eyebrow="StudioFlow · MOM" title={mom.topic} description={mom.state === "DRAFT" ? "Editable project meeting draft" : "Immutable project meeting record"} meta={<span className="text-xs text-ink-tertiary">{mom.state}{mom.sequence ? ` · MOM ${mom.sequence}` : ""}{mom.issued_at ? ` · issued ${new Intl.DateTimeFormat(undefined, { dateStyle: "medium" }).format(mom.issued_at)}` : ""}</span>} actions={<div className="flex gap-2 print:hidden"><Link href={`/studioflow/projects/${id}`} className="rounded-action px-2 py-1 text-sm font-medium text-action hover:underline">Back to project</Link><PrintButton /></div>} divider />
