...
export class BrandService {
  ...
  list(context: MasterDataExecutionContext, filter: Parameters<BrandRepository["list"]>[1] = {}) { requirePermission(context.grants, MASTERDATA_BRAND_READ); return runInMasterDataTransaction(context, this.ports.runTransaction, tx => this.ports.brands.list(tx, filter)); }
  create(context: MasterDataExecutionContext, input: BrandWriteInput) { requirePermission(context.grants, MASTERDATA_BRAND_MANAGE); return runInMasterDataTransaction(context, this.ports.runTransaction, tx => this.write(context, tx, null, input)); }
  ...
  private async write(context: MasterDataExecutionContext, tx: TransactionClient, current: BrandRecord | null, input: BrandWriteInput): Promise<BrandRecord> {
    ...
    const graph: BrandGraphInput = {
      id: current?.id ?? input.requestedId ?? this.ports.generateId(), name, slug, ownerPartyId, notes: optionalText(input.notes),
    ...
    await this.ports.auditWriter.write(prepareAuditEvent({ appId: "masterdata", action: current ? "brand.updated" : "brand.created", entityType: "brand", entityId: saved.id, actor: context.actor, requestId: context.requestId, occurredAt: this.ports.now(), changes }), tx); return saved;
  }
  softDelete(context: MasterDataExecutionContext, id: string) { requirePermission(context.grants, MASTERDATA_BRAND_MANAGE); return runInMasterDataTransaction(context, this.ports.runTransaction, async tx => { const current = await this.ports.brands.findById(tx, id); if (!current || current.deletedAt) throw new AppError("NOT_FOUND", "BRAND_NOT_FOUND", "This Brand no longer exists."); assertBrandCanBeDeleted(await this.ports.brands.countDeleteReferences(tx, id)); const at = this.ports.now(); const saved = await this.ports.brands.setDeletedAt(tx, id, at); await this.ports.auditWriter.write(prepareAuditEvent({ appId: "masterdata", action: "brand.deleted", entityType: "brand", entityId: id, actor: context.actor, requestId: context.requestId, occurredAt: at, changes: { deletedAt: { from: null, to: at } } }), tx); return saved; }); }
  restore(context: MasterDataExecutionContext, id: string) { requirePermission(context.grants, MASTERDATA_BRAND_MANAGE); return runInMasterDataTransaction(context, this.ports.runTransaction, async tx => { const current = await this.ports.brands.findById(tx, id); if (!current) throw new AppError("NOT_FOUND", "BRAND_NOT_FOUND", "This Brand no longer exists."); if (!current.deletedAt) throw new AppError("CONFLICT", "BRAND_NOT_DELETED", "Only a deleted Brand can be restored."); const conflict = await this.ports.brands.findLiveIdentityConflict(tx, current.name, current.slug); if (conflict && conflict.id !== id) throw new AppError("CONFLICT", "BRAND_IDENTITY_TAKEN", "The Brand identity is already used."); assertBrandCategories(await this.ports.brands.loadCategoryCandidates(tx, current.categories.map(x => x.categoryId))); assertBrandSuppliers(await this.ports.brands.loadSupplierCandidates(tx, current.suppliers.map(x => x.partyId))); if (current.ownerPartyId && !(await this.ports.brands.isLiveOwnerCandidate(tx, current.ownerPartyId))) throw new AppError("CONFLICT", "BRAND_OWNER_INVALID", "The Brand owner is no longer live."); const saved = await this.ports.brands.setDeletedAt(tx, id, null); await this.ports.auditWriter.write(prepareAuditEvent({ appId: "masterdata", action: "brand.restored", entityType: "brand", entityId: id, actor: context.actor, requestId: context.requestId, occurredAt: this.ports.now(), changes: { deletedAt: { from: current.deletedAt, to: null } } }), tx); return saved; }); }
}
