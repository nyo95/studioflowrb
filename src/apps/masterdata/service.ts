...
/**
 * Permanently removes one already-archived Master Data entity for the direct
 * deletion-approver shortcut. The request approval path retains its request
 * metadata and guards in the approval transaction below.
 */
async function hardDeleteMasterDataTarget(
  tx: TxClient,
  targetType: string,
  targetId: string,
): Promise<MasterDataDeletionTarget> {
  if (![
    "brand",
    "vendor",
    "sku",
  ...
  const target = targetType as MasterDataDeletionTarget;
  ...
  if (target === "brand") {
    ...
    if (supplierCount > 0) throw new AppError("CONFLICT", "BRAND_HAS_SUPPLIERS", "Brand still has supplier relations. Remove them first.");
    const contactCount = await tx.vendorContact.count({ where: { brand_id: targetId } });
    if (contactCount > 0) throw new AppError("CONFLICT", "BRAND_HAS_CONTACTS", "Brand still has scoped contacts. Remove them first.");
    ...
    const bcIds = await tx.brandCategory.findMany({ where: { brand_id: targetId }, select: { id: true } }).then((rows) => rows.map((row) => row.id));
    if (bcIds.length > 0) {
    ...
    const priceIds = await tx.priceMaterial.findMany({ where: { sku_id: targetId }, select: { id: true } }).then((rows) => rows.map((row) => row.id));
    if (priceIds.length > 0) {
      await tx.archiveCause.deleteMany({ where: { entity_type: "price_material", entity_id: { in: priceIds } } });
      await tx.priceMaterial.deleteMany({ where: { id: { in: priceIds } } });
    }
    ...
    if (brandCatCount + skuCatCount + laborCount + workPriceCount > 0) throw new AppError("CONFLICT", "CATEGORY_HAS_DEPENDENCIES", `Category is still referenced by ${brandCatCount + skuCatCount + laborCount + workPriceCount} record(s) and cannot be permanently deleted.`);
    try { await tx.category.delete({ where: { id: targetId } }); } catch (error) { mapWriteError(error); }
  } else if (target === "vendor_type") {
    ...
    const assignmentCount = await tx.vendorVendorType.count({ where: { vendor_type_id: targetId } });
  ...
  } else if (target === "price_material_labor") {
...
export function createMasterDataService(db: PrismaClient, ports: MasterDataServicePorts) {
  ...
  const directDeleteOrNull = async (input: {
    grants: PermissionGrants;
    actor: AuditActor;
    targetType: MasterDataDeletionTarget;
    targetId: string;
  }) => {
    ...
    return runTransaction(async (tx) => {
      ...
      await writeAudit(tx, {
        action: `${targetType.replace(/_/g, "-")}.deleted`,
        entityType: targetType,
        entityId: input.targetId,
        actor: input.actor,
        metadata: { deletion_mode: "direct", approver_user_id: input.actor.userId, approver_label: input.actor.label },
      });
  ...
  return {
    ...
    async hardDeleteArchived(input: {
      grants: PermissionGrants;
      actor: AuditActor;
      targetType: MasterDataDeletionTarget;
      targetId: string;
    ...
    async approveDeletion(input: {
      ...
      return runTransaction(async (tx) => {
        ...
        if (targetType === "brand") {
          ...
          const activeSkuCount = await tx.sku.count({ where: { brand_id: targetId, deleted_at: null } });
          if (activeSkuCount > 0) {
            throw new AppError("CONFLICT", "BRAND_HAS_ACTIVE_SKUS", "Brand still has active SKUs. Archive them before permanent deletion.");
          }
          const priceIds = skuIds.length === 0 ? [] : await tx.priceMaterial.findMany({ where: { sku_id: { in: skuIds } }, select: { id: true } }).then((rows) => rows.map((row) => row.id));
          if (priceIds.length > 0) {
            await tx.archiveCause.deleteMany({ where: { entity_type: "price_material", entity_id: { in: priceIds } } });
