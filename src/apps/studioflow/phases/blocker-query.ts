...
export async function readBlockerCounts(client: Db | TxClient, phaseId: string): Promise<PhaseBlockerCounts> {
  ...
  const [openRevisionActivities, openDeferredActivities, openRootChecklistItems] = await Promise.all([
  ...
  return { openRevisionActivities, openDeferredActivities, openRootChecklistItems };
}
